      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 space-y-8">
          <div className="rounded-2xl border border-white/10 bg-card p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm text-gray-300">{t("countryLabel")}</label>
                  <input
                    value={form.locationCountry}
                    onChange={(event) => setForm((prev) => ({ ...prev, locationCountry: event.target.value }))}
                    className="w-full rounded-xl border border-white/10 bg-zinc-900/60 text-white px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40"
                    placeholder={t("countryPlaceholder")}
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-sm text-gray-300">{t("cityLabel")}</label>
                  <input
          ...
          <div className="rounded-2xl border border-white/10 bg-card p-6">
            ...
            {data.works.length === 0 ? (
              <div className="rounded-xl border border-dashed border-white/10 py-12 text-center text-gray-400">
                {t("noWorks")}
                <div className="mt-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {data.works.map((work) => (
                  <Link
                    ...
                    className="rounded-xl border border-white/10 bg-zinc-900/40 p-4 hover:border-primary/40 hover:bg-zinc-900/60 transition-colors"
                  >
                    <div className="aspect-video rounded-lg overflow-hidden bg-zinc-900 mb-3">
                      {work.coverUrl ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-xs text-gray-500">{t("noCover")}</div>
                      )}
                    </div>
                    <h3 className="text-white font-semibold line-clamp-1">{work.title}</h3>
                    <p className="text-sm text-gray-400 mt-1 line-clamp-2 min-h-[40px]">{work.summary || t("noSummary")}</p>
                    <div className="mt-3 flex items-center justify-between text-xs text-gray-500">
                      <span className="inline-flex items-center gap-1">
                        <MapPin className="w-3 h-3" />
                        {[work.countryCode, work.cityCode].filter(Boolean).join(" / ") || t("na")}
                      </span>
                      <span>{new Date(work.createdAt).toLocaleDateString(locale)}</span>
                    </div>
                    <div className="mt-2 text-xs text-gray-500">{t("worksMeta", { views: work.views, likes: work.likes })}</div>
                  </Link>
                ))}
              </div>
            )}
