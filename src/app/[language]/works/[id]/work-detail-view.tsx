...
export function WorkDetailView() {
  ...
  const teamMembers = Array.isArray(work?.team)
    ? work.team
    : typeof work?.team === 'string'
      ? work.team.split(',').map(member => member.trim()).filter(Boolean)
      : [];
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-6 text-sm">
                <div className="flex items-center gap-2 text-gray-400">
                  ...
                  <span className="text-gray-200">{teamMembers.join(", ")}</span>
                </div>
                <div className="flex items-center gap-2 text-gray-400">
        ...
        <div className="md:col-span-2 space-y-8">
          ...
          <section className="bg-card p-8 rounded-2xl shadow-sm border border-border">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(work.screenshots || []).map((url, index) => (
                <img
                  key={index}
                  src={url}
