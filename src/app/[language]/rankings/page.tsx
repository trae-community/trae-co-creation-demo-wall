    ...
    <div className={cn("flex items-center flex-wrap", small ? "gap-3 text-xs" : "gap-4 text-sm")}>
      {stats.map(({ icon: Icon, label, value }) => (
        <span key={label} className="flex items-center gap-1.5">
          {Icon ? (
            <Icon className={cn("text-muted-foreground", small ? "w-3 h-3" : "w-3.5 h-3.5")} />
          ) : (
...
function TopBarCard({ item, rank }: { item: PodiumItem; rank: 0 | 1 | 2 }) {
  ...
  return (
    ...
    >
      ...
      {hasCover ? (
        <div className="shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-xl overflow-hidden bg-muted">
          <img src={item.image!} alt={item.title} className="w-full h-full object-cover" />
        </div>
      ) : isCity ? (
        <div className="shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-xl overflow-hidden bg-muted/50 flex items-center justify-center">
        ...
        <div className="shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden bg-muted/50 flex items-center justify-center">
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.subtitle && <p className="text-muted-foreground text-xs truncate mt-1">{item.subtitle}</p>}
      </div>
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex justify-center gap-2">
          {[1, 2, 3].map(i => <div key={i} className="h-10 w-28 rounded-full bg-muted animate-pulse" />)}
        </div>
          ...
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="rounded-xl border border-border p-4 flex items-center gap-4">
              ...
              <div className="flex gap-4">
                {[1, 2, 3].map(j => <div key={j} className="h-8 w-12 bg-muted rounded animate-pulse" />)}
              </div>
            </div>
          ))}
        </div>
            ...
            sortedCities.slice(3).map((city, idx) => {
              ...
              return (
                <div
                  key={city.code}
                  className={cn(
                    "rounded-xl border p-4 flex items-center gap-4 transition-all hover:border-border",
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {city.province && (
                        <span className="text-xs text-muted-foreground">{localizedProvinceName(city)}</span>
                      )}
                    </div>
                    {/* Progress bar */}
                  ...
                  <div className="flex items-center gap-4 shrink-0 text-sm">
                    <div className={cn("flex items-center gap-1.5", sortKey !== 'works' && "opacity-50")}>
                      <FileText className={cn("w-3.5 h-3.5", sortKey === 'works' ? "text-primary" : "text-muted-foreground")} />
                    ...
                    <div className={cn("flex items-center gap-1.5", sortKey !== 'views' && "opacity-50")}>
                      <Eye className={cn("w-3.5 h-3.5", sortKey === 'views' ? "text-primary" : "text-muted-foreground")} />
                    ...
                    <div className={cn("flex items-center gap-1.5", sortKey !== 'likes' && "opacity-50")}>
                      <ThumbsUp className={cn("w-3.5 h-3.5", sortKey === 'likes' ? "text-primary" : "text-muted-foreground")} />
          ...
          sortedWorks.length === 0 ? (
            ...
            sortedWorks.slice(3).map((work, idx) => (
              <div
                ...
                onClick={() => router.push(`/${locale}/works/${work.id}?from=${encodeURIComponent(`/${locale}/rankings`)}`)}
                className={cn(
                  "rounded-xl border p-4 flex items-center gap-4 transition-all hover:border-border cursor-pointer group",
                  rankRowBg(idx + 3)
                )}
              >
                {/* Rank */}
                ...
                {/* Cover */}
                <div className="shrink-0 w-14 h-14 rounded-lg overflow-hidden border border-border bg-muted">
                  {work.coverUrl ? (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{t('author')}: <Link href={`/user/${work.author.id}?from=${encodeURIComponent(`/${locale}/rankings`)}`} onClick={(e) => e.stopPropagation()} className="hover:text-primary transition-colors">{work.author.name}</Link></p>
                </div>
          ...
          sortedCreators.length === 0 ? (
            ...
            sortedCreators.slice(3).map((creator, idx) => (
              ...
              >
                ...
                {/* Avatar */}
                <div className="shrink-0 w-12 h-12 rounded-full overflow-hidden border border-border bg-muted">
                  {creator.avatarUrl ? (
        ...
        {mainTab === 'trending' && (
          data?.trendingWorks.length === 0 ? (
            <div className="text-center py-16 rounded-2xl border border-dashed border-border bg-card">
              ...
              <p className="text-muted-foreground text-sm">{t('noTrendingData')}</p>
            </div>
          ) : (
            <div className="space-y-2">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-xs text-muted-foreground">{work.author.name}</span>
                      {work.createdAt && (
